...
function runSuite(name, options, callback) {
  var version = options.version || require('./VERSION');
  var errors = [];
  var iterator = new Iterator(path.join(CACHE_DIR, version, 'test', name), {
  ...
  iterator.forEach(
    function (entry, callback) {
      ...
      spawn('node', RUNTIME_ARGS.concat([TEST_RUNNER, scopedPath, '--version', version]), { stdio: 'inherit', cwd: CACHE_DIR }, function (err, res) {
        if (err) {
          console.log(err);
          errors.push(err);
...
module.exports = function run(options, callback) {
  ...
  for (var key in TEST_SUITES) queue.defer(runSuite.bind(null, key, options));
