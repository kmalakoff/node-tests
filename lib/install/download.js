...
module.exports = function download(target, options, callback) {
  var repository = options.repository || 'https://codeload.github.com/nodejs/node/zip/';
  var tempFile = path.join(path.dirname(target), uuid.v4());

  var queue = new Queue(1);
  queue.defer(function (callback) {
    // eslint-disable-next-line node/no-deprecated-api
    var parsed = url.parse(repository + options.version);
    ...
    req.on('response', function (res) {
      ...
      eos(res.pipe(progress).pipe(fs.createWriteStream(tempFile)), callback);
    });
    req.end();
  });
  queue.defer(function (callback) {
