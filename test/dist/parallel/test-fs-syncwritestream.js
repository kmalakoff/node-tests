...
var path = require('path'); // require('internal/fs/utils').SyncWriteStream is used as a stdio
...
if (process.argv[2] === 'child') {
  ...
  console.log(JSON.stringify([process.stdout, process.stderr].map(function (stdio) {
    return {
...
tmpdir.refresh();
var filename = path.join(tmpdir.path, 'stdout');
var stdoutFd = fs.openSync(filename, 'w');
var proc = spawn(process.execPath, [__filename, 'child'], {
  stdio: ['inherit', stdoutFd, stdoutFd]
});
proc.on('close', common.mustCall(function () {
