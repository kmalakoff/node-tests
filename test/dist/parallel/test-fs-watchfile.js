...
var tmpdir = require('../common/tmpdir'); // Basic usage tests.
...
var expectedStatObject = new fs.Stats(0, // dev
0, // mode
0, // nlink
0, // uid
0, // gid
...
var watcher = fs.watchFile(enoentFile, {
  interval: 0
  ...
  if (!fileExists) {
    ...
    assert(curr.ino > 0); // As the file just got created, previous ino value should be lesser than
    // or equal to zero (non-existent file).

    assert(prev.ino <= 0); // Stop watching the file
...
watcher.on('stop', common.mustCall(function onStop() {})); // Watch events should callback with a filename on supported systems.
// Omitting AIX. It works but not reliably.

if (common.isLinux || common.isOSX || common.isWindows) {
  var dir = path.join(tmpdir.path, 'watch');
